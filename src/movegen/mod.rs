...
mod test {
    ...
    fn calculates_opponent_vision() {
        ...
        let opponent_vision = game.generate_vision(&Turn::White).unwrap();
    ...
    fn pseudolegal_move_count_correct_starting_position() {
        ...
        let moves = game.generate_pseudolegal_moves().unwrap();
    ...
    fn legal_move_count_correct_starting_position() {
        let game = Game::new_default();
        let moves = game.generate_legal_moves().unwrap();
        assert_eq!(moves.len(), 20);
    }
    ...
    fn calculates_correct_moves_advanced_position() {
        ...
        assert!(!moves.contains(&LongAlgebraicMove::from_algebraic("a1b1").unwrap()));
    }

    #[bench]
    fn benchmark_movegen(b: &mut Bencher) {
        b.iter(|| {
            Game::new_default().generate_legal_moves().unwrap();
        })
    }
}
